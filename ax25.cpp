...
// Module functions
static void
update_crc(const uint8_t a_bit) 
{
...
void
ax25_send_bit(const uint8_t a_bit, const uint8_t bitStuff)
{
    if (bitStuff) update_crc(a_bit);
    if (a_bit) {
...
void
ax25_send_string(const char *const string)
{
...
void
ax25_initBuffer(uint8_t *buf, const int bufSize)
{
    packet = buf;
...
void
ax25_send_header(const struct PathAddress *const paths, const int nPaths, const uint16_t txDelay)
{
    
    int i, j;
    
    for (i = 0; i < txDelay * 3 / 20; i++) {
        ax25_send_flag();
    }
