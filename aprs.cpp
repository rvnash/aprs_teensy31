...
{
    afsk_setup(p_txDelay, p_pttPin, p_pttDelay, p_toneLength, p_silenceLength);
}
...
void aprs_send(const PathAddress *const paths, const int nPaths,
               ...
               const float lat, const float lon, // degrees
               ...
               const char *const comment)
{
    ...
    ax25_send_header(paths, nPaths, afsk_getTxDelay());
    
    ax25_send_byte('/');                // Report w/ timestamp, no APRS messaging. $ = NMEA raw data
    snprintf(temp, sizeof(temp), "%02u%02u%02uz", (unsigned int)dayOfMonth, (unsigned int)hour, (unsigned int)min); 
